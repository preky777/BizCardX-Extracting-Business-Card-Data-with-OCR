...
# Function to fetch data from the database and return a DataFrame
...
def ext_mod_page():
    ...
    # Section 2: Modify
        ...
        try:
            ...
            cursor.execute("SELECT company_name, card_holder_name, designation, mobile_number, email_address, website, area, city, state, pincode FROM business_cards WHERE card_holder_name=%s",
                        (selected_card,))
            ...
            email = st.text_input("Email Address", result[4])
            ...
            if st.button("Save changes to DB"):
                ...
                cursor.execute("""UPDATE business_cards SET company_name=%s, card_holder_name=%s, designation=%s, mobile_number=%s, email_address=%s, website=%s, area=%s, city=%s, state=%s, pincode=%s
                                WHERE card_holder_name=%s""", (company_name, card_holder, designation, mobile_number, email, website, area, city, state, pincode, selected_card))
                conn.commit()
        ...
        if st.button("View updated data"):
            cursor.execute("SELECT company_name, card_holder_name, designation, mobile_number, email_address, website, area, city, state, pincode FROM business_cards")
            updated_data = cursor.fetchall()
            if updated_data:
                updated_df = pd.DataFrame(updated_data, columns=["Company Name", "Card Holder Name", "Designation", "Mobile Number", "Email Address", "Website", "Area", "City", "State", "PinCode"])
                st.write(updated_df)
            else:
        ...
        if st.button("Yes, Delete Business Card"):
            cursor.execute(f"DELETE FROM business_cards WHERE card_holder_name='{selected_card}'")
            conn.commit()
            st.success("Business card information deleted from the database.")
